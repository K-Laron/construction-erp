    ...
    <div className="flex-1 p-6 space-y-6 overflow-y-auto no-print">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">Manage credit accounts, ledgers, and payments</p>
        </div>
        <button
              ...
              {filteredCustomers.map(c => {
                ...
                return (
                  <>
                    <tr
                      key={c.id}
                      className={`border-b border-slate-850 hover:bg-slate-900/40 transition-colors ${
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-slate-400">{c.phone || 'N/A'}</td>
                      ...
                      <td className="py-3.5 px-4 text-slate-300 font-mono font-medium">{formatCurrency(c.credit_limit)}</td>
                    ...
                    {isExpanded && (
                      <tr>
                        <td colSpan={6} className="bg-slate-900/60 p-4 border-b border-slate-800">
                          <div className="space-y-4">
                            ...
                            {ledgerLoading ? null : ledgerData.length === 0 ? (
                              <p className="text-slate-500 text-xs">No historical ledger transactions.</p>
                            ) : (
                                  ...
                                  <tbody>
                                    {ledgerData.map(entry => (
                                      <tr key={entry.id} className="border-b border-slate-850 hover:bg-slate-950/20">
                                        <td className="py-2 px-3 text-slate-500">{formatDate(entry.date)}</td>
                                        <td className="py-2 px-3 text-slate-300">{entry.description}</td>
                                        ...
                                        <td className="py-2 px-3 text-center text-slate-500 font-mono text-[9px] truncate max-w-[120px]">
